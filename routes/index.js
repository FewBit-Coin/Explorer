				...
				if (tx && tx != "There was an error. Check your console.") {
					lib.prepare_vin(tx, function (vin, tx_type_vin) {
						lib.prepare_vout(
							...
							block.tx[i],
							vin,
							!settings.blockchain_specific.zksnarks.enabled ||
								typeof tx.vjoinsplit === "undefined" ||
								tx.vjoinsplit == null
								? []
								: tx.vjoinsplit,
							function (vout, nvin, tx_type_vout) {
		...
		if (block && block != "There was an error. Check your console.") {
			...
			if (!isNaN(blockhash)) {
				...
				lib.get_blockhash(height, function (hash) {
					if (
						hash &&
						hash != "There was an error. Check your console."
					)
						res.redirect("/block/" + hash);
					else route_get_txlist(res, "Block not found: " + blockhash);
				});
			} else route_get_txlist(res, "Block not found: " + blockhash);
		}
	});
		...
		db.get_tx(txid, function (tx) {
			if (tx) {
				...
				lib.get_rawtransaction(txid, function (rtx) {
					if (rtx && rtx.txid) {
						lib.prepare_vin(rtx, function (vin, tx_type_vin) {
							lib.prepare_vout(
								...
								rtx.txid,
								vin,
								!settings.blockchain_specific.zksnarks
									.enabled ||
									typeof rtx.vjoinsplit === "undefined" ||
									rtx.vjoinsplit == null
									? []
									: rtx.vjoinsplit,
								function (rvout, rvin, tx_type_vout) {
									lib.calculate_total(
										rvout,
										function (total) {
													...
													function (block) {
														if (
															...
															if (
																...
																db.populate_claim_address_names(
																	utx,
																	function (
																		utx
																	) {
					...
					} else route_get_txlist(res, null);
				});
			}
		});
	}
...
function route_get_txlist(res, error) {
	...
	get_last_updated_date(
		settings.index_page.page_header.show_last_updated,
...
function route_get_address(res, hash) {
	...
	if (
		hash != null &&
		hash.toLowerCase() != "coinbase" &&
		((hash.toLowerCase() == "hidden_address" &&
			settings.address_page.enable_hidden_address_view == true) ||
			(hash.toLowerCase() == "unknown_address" &&
				settings.address_page.enable_unknown_address_view == true) ||
			(hash.toLowerCase() != "hidden_address" &&
				hash.toLowerCase() != "unknown_address"))
	) {
		...
		db.get_address(hash, false, function (address) {
			if (address) {
				...
				} else send_address_data(res, address, null);
			} else route_get_txlist(res, hash + " not found");
		});
	} else route_get_txlist(res, hash + " not found");
}
...
function route_get_claim_form(res, hash) {
	...
	} else route_get_address(res, hash);
}
...
router.get("/markets/:market/:coin_symbol/:pair_symbol", function (req, res) {
	...
	if (settings.markets_page.enabled == true) {
		...
		if (
			...
			settings.markets_page.exchanges[market_id].trading_pairs.findIndex(
				(p) =>
					p.toLowerCase() ==
					coin_symbol.toLowerCase() + "/" + pair_symbol.toLowerCase()
			) > -1
		) {
			// lookup market data
			db.get_market(market_id, coin_symbol, pair_symbol, function (data) {
				...
				if (
					market_data.market_url_template != null &&
					market_data.market_url_template != ""
				) {
					switch (
						market_data.market_url_case == null ||
						market_data.market_url_case == ""
							? "l"
							: market_data.market_url_case.toLowerCase()
					) {
						case "l":
						case "lower":
							url = market_data.market_url_template
								.replace("{base}", pair_symbol.toLowerCase())
								.replace("{coin}", coin_symbol.toLowerCase())
								.replace(
									"{url_prefix}",
									market_data.market_url != null
										? market_data.market_url({
												coin: coin_symbol.toLowerCase(),
												exchange:
							...
							isAlt =
								market_data.isAlt != null
									? market_data.isAlt({
											coin: coin_symbol.toLowerCase(),
											exchange: pair_symbol.toLowerCase(),
									  })
									: false;
							break;
						case "u":
						case "upper":
							url = market_data.market_url_template
								.replace("{base}", pair_symbol.toUpperCase())
								.replace("{coin}", coin_symbol.toUpperCase())
								.replace(
									"{url_prefix}",
									market_data.market_url != null
										? market_data.market_url({
							...
							isAlt =
								market_data.isAlt != null
									? market_data.isAlt({
											coin: coin_symbol.toUpperCase(),
											exchange: pair_symbol.toUpperCase(),
									  })
									: false;
							break;
						default:
				...
				var market_name = isAlt
					? market_data.market_name_alt == null
						? ""
						: market_data.market_name_alt
					: market_data.market_name == null
					? ""
					: market_data.market_name;
				var market_logo = isAlt
					? market_data.market_logo_alt == null
						? ""
						: market_data.market_logo_alt
					: market_data.market_logo == null
					? ""
					: market_data.market_logo;
				var marketdata = {
				...
				// lookup the last updated date if necessary
				get_last_updated_date(
					settings.markets_page.page_header.show_last_updated,
					...
					function (last_updated_date) {
						res.render("./market", {
							...
							styleHash: get_style_hash(),
							themeHash: get_theme_hash(),
							page_title_prefix: locale.mkt_title.replace(
								"{1}",
					...
					db.get_distribution(
						...
						function (distribution) {
							res.render("richlist", {
								...
								last_updated:
									settings.richlist_page.page_header
										.show_last_updated == true
										? stats.richlist_last_updated
										: null,
								showSync: db.check_show_sync_message(),
								customHash: get_custom_hash(),
								styleHash: get_style_hash(),
...
router.get("/movement", function (req, res) {
	...
	if (settings.movement_page.enabled == true) {
		// lookup the last updated date if necessary
		get_last_updated_date(
			settings.movement_page.page_header.show_last_updated,
			"blockchain_last_updated",
			function (last_updated_date) {
				res.render("movement", {
					active: "movement",
					...
					themeHash: get_theme_hash(),
					page_title_prefix:
						settings.coin.name + " " + "Coin Movements",
				});
			}
		);
	} else {
...
router.get("/network", function (req, res) {
	...
	if (settings.network_page.enabled == true) {
		// lookup the last updated date if necessary
		get_last_updated_date(
			settings.network_page.page_header.show_last_updated,
			...
			function (last_updated_date) {
				res.render("network", {
					...
					themeHash: get_theme_hash(),
					page_title_prefix:
						settings.coin.name + " " + "Network Peers",
				});
			}
		);
	} else {
...
router.get("/masternodes", function (req, res) {
	...
	if (settings.masternodes_page.enabled == true) {
		// lookup the last updated date if necessary
		get_last_updated_date(
			settings.masternodes_page.page_header.show_last_updated,
...
router.get("/reward", function (req, res) {
	...
	if (
		settings.blockchain_specific.heavycoin.enabled == true &&
		settings.blockchain_specific.heavycoin.reward_page.enabled == true
	) {
		db.get_stats(settings.coin.name, function (stats) {
			db.get_heavy(settings.coin.name, function (heavy) {
				...
				res.render("reward", {
					...
					votes: votes,
					last_updated:
						settings.blockchain_specific.heavycoin.reward_page
							.page_header.show_last_updated == true
							? stats.reward_last_updated
							: null,
					showSync: db.check_show_sync_message(),
					customHash: get_custom_hash(),
					styleHash: get_style_hash(),
					themeHash: get_theme_hash(),
					page_title_prefix:
						settings.coin.name + " Reward/Voting Details",
				});
			});
		});
	} else {
...
router.get("/orphans", function (req, res) {
	...
	if (settings.orphans_page.enabled == true) {
		// lookup the last updated date if necessary
		get_last_updated_date(
			settings.orphans_page.page_header.show_last_updated,
			"blockchain_last_updated",
			function (last_updated_date) {
				res.render("orphans", {
					active: "orphans",
					...
					styleHash: get_style_hash(),
					themeHash: get_theme_hash(),
					page_title_prefix: locale.orphan_title.replace(
						"{1}",
						settings.coin.name
					),
				});
	...
	if (settings.shared_pages.page_header.search.enabled == true) {
		...
		if (query.length == 64) {
			...
			db.get_address(query, false, function (address) {
				if (address) res.redirect("/address/" + address.a_id);
				else {
