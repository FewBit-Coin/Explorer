...
const rpc_queue = async.queue((task_params, cb) => {
	client.cmd([task_params], function (err, response) {
		if (err) return cb("There was an error. Check your console.");
...
function coinbase_supply(cb) {
	Address.findOne({ a_id: "coinbase" })
		.then((address) => {
			if (address) return cb(address.sent);
			else return cb(0);
		})
		.catch((err) => {
			console.log(err);
...
function rpcCommand(params, cb) {
	rpc_queue.push(
		{ method: params[0].method, params: params[0].parameters },
		cb
	);
}
...
function prepareRpcCommand(cmd, addParams) {
	...
	if (cmd != null && cmd.trim() != "") {
		...
		for (i = 0; i < split.length; i++) {
			if (i == 0) method_name = split[i];
			else params.push(split[i]);
		}
	}
...
function convertHashUnits(hashes) {
	if (
		settings.shared_pages.page_header.panels.network_panel.nethash_units ==
		"K"
	) {
...
function encodeP2PKaddress(p2pk_descriptor, cb) {
	...
	module.exports.get_descriptorinfo(
		p2pk_descriptor,
		function (descriptor_info) {
			// check for errors
			if (descriptor_info != null) {
				// encode the address using the output descriptor
				module.exports.get_deriveaddresses(
					descriptor_info.descriptor,
					function (p2pkh_address) {
						// check for errors
	...
	if (raw_count != null) {
		...
		if (raw_count.total != null && raw_count.enabled != null) {
			...
			if (regex.test(raw_count)) {
				...
				raw_count.toString().indexOf("Total: ") > -1 &&
				raw_count.toString().indexOf("Enabled: ")
			) {
...
module.exports = {
	...
	get_hashrate: function (cb) {
		...
		if (
			settings.shared_pages.page_header.panels.network_panel.nethash ==
			"netmhashps"
		) {
			...
			if (!(cmd.method == "" && cmd.parameters.length == 0)) {
				...
				if (settings.api_cmds.use_rpc) {
					...
					rpcCommand(
						[{ method: cmd.method, parameters: cmd.parameters }],
						function (response) {
	...
	get_masternodecount: function (cb) {
		...
		if (
			settings.api_page.public_apis.rpc.getmasternodecount.enabled ==
				true &&
			settings.api_cmds["getmasternodecount"] != null &&
			settings.api_cmds["getmasternodecount"] != ""
		) {
					...
					rpcCommand(
						...
						function (response) {
							...
							if (
								...
								return cb(null);
							else return cb(normalizeMasternodeCount(response));
						}
					);
				} else {
					...
					request(
						...
						function (error, response, body) {
							...
							if (
								...
								return cb(null);
							else return cb(normalizeMasternodeCount(body));
						}
					);
				}
			} else {
	...
	get_blockhash: function (height, cb) {
		var cmd = prepareRpcCommand(
			settings.api_cmds.getblockhash,
		...
		if (!(cmd.method == "" && cmd.parameters.length == 0)) {
			if (settings.api_cmds.use_rpc) {
				...
				var uri =
					base_url + "getblockhash?height=" + (height ? height : "");
	...
	get_rawtransaction: function (hash, cb) {
		var cmd = prepareRpcCommand(
			settings.api_cmds.getrawtransaction,
			hash ? [hash, 1] : []
		);
		...
		if (!(cmd.method == "" && cmd.parameters.length == 0)) {
			if (settings.api_cmds.use_rpc) {
				...
				var uri =
					base_url + "getrawtransaction?txid=" + hash + "&decrypt=1";
	...
	get_maxmoney: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getmaxmoney
		);
	...
	get_maxvote: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getmaxvote
		);
	...
	get_vote: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getvote
		);
	...
	get_phase: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getphase
		);
	...
	get_reward: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getreward
		);
	...
	get_estnext: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds
	...
	get_nextin: function (cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.heavycoin.api_cmds.getnextrewardwhenstr
		);
	...
	get_descriptorinfo: function (descriptor, cb) {
		...
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.bitcoin.api_cmds.getdescriptorinfo,
	...
	get_deriveaddresses: function (descriptor, cb) {
		var cmd = prepareRpcCommand(
			settings.blockchain_specific.bitcoin.api_cmds.deriveaddresses,
			descriptor ? [descriptor] : []
		);
	...
	get_supply: function (cb) {
		if (settings.sync.supply == "HEAVY") {
			...
			var cmd = prepareRpcCommand(
				settings.blockchain_specific.heavycoin.api_cmds.getsupply
			);
				...
				if (settings.api_cmds.use_rpc) {
					rpcCommand(
						...
						function (response) {
							...
							if (
								!response ||
								!response.moneysupply ||
								response ==
									"There was an error. Check your console."
							)
								return cb(null);
							else return cb(response.moneysupply);
						}
					);
				} else {
					...
					request(
						...
						function (error, response, body) {
							...
							if (
								...
								return cb(null);
							else return cb(body.moneysupply);
						}
					);
				}
			} else {
					...
					rpcCommand(
						...
						function (response) {
							...
							if (
								...
								return cb(null);
							else return cb(response.total_amount);
						}
					);
				} else {
					...
					request(
						...
						function (error, response, body) {
							...
							if (
								...
								return cb(null);
							else return cb(body.total_amount);
						}
					);
				}
			} else {
	...
	verify_message: function (address, signature, message, cb) {
		var cmd = prepareRpcCommand(settings.api_cmds.verifymessage, [
			address,
			signature,
			message,
		]);
	...
	get_geo_location: function (address, cb) {
		request(
			{ uri: "https://reallyfreegeoip.org/json/" + address, json: true },
	...
	prepare_vout: function (vout, txid, vin, vhidden, cb) {
		...
		module.exports.syncLoop(
					...
					if (vout[i].scriptPubKey.type != "zerocoinmint") {
						...
						if (address_list == null || address_list.length == 0) {
							...
							if (vout[i].scriptPubKey.address == null) {
								...
								if (
									settings.blockchain_specific.bitcoin
										.enabled == true
								) {
									...
									encodeP2PKaddress(
										...
										function (p2pkh_address) {
											...
											if (p2pkh_address != null) {
												...
												console.log(
													"Failed to find vout address from tx " +
														txid
												);
												// process vout addresses
												processVoutAddresses(
						...
						console.log(
							"Zerocoin tx found. skipping for now as it is unsupported"
						);
						tx_type = "zerocoin";
						loop.next();
					}
			...
			function () {
				...
				if (
					typeof vout[0] !== "undefined" &&
					vout[0].scriptPubKey.type == "nonstandard"
				) {
					if (arr_vin.length > 0 && arr_vout.length > 0) {
						if (arr_vin[0].addresses == arr_vout[0].addresses) {
							...
							if (arr_vin == null || arr_vin.length == 0) {
								...
								module.exports.prepare_vin(
									{
										txid: txid,
										vin: arr_vin,
										vout: new_vout,
									},
									function (return_vin, return_tx_type_vin) {
										return cb(
											arr_vout,
											return_vin,
											return_tx_type_vin
						...
						} else return cb(arr_vout, arr_vin, tx_type);
					} else return cb(arr_vout, arr_vin, tx_type);
				} else return cb(arr_vout, arr_vin, tx_type);
			}
		);
					...
					module.exports.syncLoop(
						...
						function (loop) {
							...
							if (tx.vout[i].n == input.vout) {
								if (
									tx.vout[i].scriptPubKey.addresses ||
									tx.vout[i].scriptPubKey.address
								) {
									var new_address =
										tx.vout[i].scriptPubKey.address ||
										...
										encodeP2PKaddress(
											...
											function (p2pkh_address) {
												...
												if (p2pkh_address != null) {
													...
													console.log(
														"Failed to find vin address from tx " +
															input.txid
													);
													module.exports.is_unique(
														addresses,
	...
	prepare_vin: function (tx, cb) {
		...
		module.exports.syncLoop(
			...
			function (loop) {
				...
				module.exports.get_input_addresses(
					tx.vin[i],
					tx.vout,
					function (addresses, tx_type_vin) {
						...
						if (addresses && addresses.length) {
							...
							// could not decipher the address, save as unknown and move to next vin
							console.log(
								"Failed to find vin address from tx " + tx.txid
							);
							module.exports.is_unique(
								arr_vin,
								"unknown_address",
			...
			if (fs.existsSync(pidFile)) {
				...
				switch (process.platform) {
					case "win32":
						...
						var cmdResult = execSync(
							`tasklist /FI "PID eq ${fs
				...
				if (deactivateLock) {
					...
					try {
						...
						if (!silent)
							console.log(
								`Failed to delete lock file ${pidFile}: ${err}`
							);
					}
				} else {
					// script is running
	...
	get_market_currency_code: function () {
		...
		if (settings.markets_page.market_price == "COINGECKO") {
			currency =
				settings.markets_page.coingecko_currency == null ||
				settings.markets_page.coingecko_currency == ""
					? ""
					: settings.markets_page.coingecko_currency;
		} else if (
			settings.markets_page.default_exchange.trading_pair != null &&
			settings.markets_page.default_exchange.trading_pair.indexOf("/") >
				-1
		) {
			currency =
				settings.markets_page.default_exchange.trading_pair.split(
					"/"
				)[1];
		}
